...
export const TextToSpeech: React.FC = () => {
    ...
    }, [processingState, totalChunksCount, successfulChunksCount, failedChunksCount]); // Added successfulChunksCount to deps
    ...
    const removeChunk = useCallback((chunkId: string) => {
        setChunks(prevChunks => {
            const chunkToRemove = prevChunks.find(c => c.id === chunkId);
            if (chunkToRemove?.audioUrl) URL.revokeObjectURL(chunkToRemove.audioUrl);
            return prevChunks.filter(chunk => chunk.id !== chunkId);
        });
    }, []);
